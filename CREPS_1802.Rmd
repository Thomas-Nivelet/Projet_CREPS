...
## AV
Mob_inf_AV = c(8,0,mean(donnees_creps$MOBINF[POLES=="RW"]))
Mob_sup_AV = c(4,0,mean(donnees_creps$MOBSUP[POLES=="RW"]))
Stab_inf_AV = c(6,0,mean(donnees_creps$STABINF[POLES=="RW"]))
Stab_sup_AV = c(2,0,mean(donnees_creps$STABSUP[POLES=="RW"]))
data_spider_AV <- cbind(Mob_inf_AV,Mob_sup_AV,Stab_inf_AV,Stab_sup_AV)
rownames(data_spider_AV) <- c("MAX", "MIN", "RW")
colnames(data_spider_AV) <- c("Mob. inf.", "Mob. sup.", 
                              "Stab. inf.", "Stab. sup.")
data_spider_AV = as.data.frame(data_spider_AV)

## BF
Mob_inf_BF = c(8,0,mean(donnees_creps$MOBINF[POLES=="FB"]))
Mob_sup_BF = c(4,0,mean(donnees_creps$MOBSUP[POLES=="FB"]))
Stab_inf_BF = c(6,0,mean(donnees_creps$STABINF[POLES=="FB"]))
Stab_sup_BF = c(2,0,mean(donnees_creps$STABSUP[POLES=="FB"]))
data_spider_BF <- cbind(Mob_inf_BF,Mob_sup_BF,Stab_inf_BF,Stab_sup_BF)
rownames(data_spider_BF) <- c("MAX", "MIN", "FB")
...
colnames(data_spider) <- c("Downtime_1","Downtime_2", "Downtime_3")
...
radarchart(data_spider,axistype = 1,pcol = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen",  "darkorange2", "brown4"),plwd = 6, plty = 1,cglcol = "grey", cglty = 1, cglwd = 4,
axislabcol = "black",vlcex = 2.7, vlabels = colnames(data_spider),caxislabels = NULL, title = "Downtime types distribution per pole", cex.main = 3.2, calcex = 2.2)
...
legend(x = "right", legend = rownames(data_spider[-c(1,2),]), horiz = FALSE,bty = "n", pch = 20 , col = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen", "darkorange2", "brown4"),text.col = "black", cex = 3.5, pt.cex = 4)
```
...
par(mar=c(5,5,2,2))
matplot(x,y,type="l", ylim=range(y), col=c("black","red", "green3","blue","cyan","magenta", "yellow","gray"), lty=1:3, xaxt="n", ylab="Mean of Injuries", xlab="Type of downtime", main="Average number of injuries per downtime category, for each centre", cex.main=2, cex.lab = 1.8, cex.axis = 1.6, lwd = 4)
legend('topright',legend = c("RW", "FB", "BK", "BL", "BS", "S", "SS", "R"), col = c("black","red", "green3","blue","cyan","magenta", "yellow","gray"), lty = 1:3, lwd = 4, ncol = 2, cex = 2)
xtick=seq(1,3, by=1)
axis(side=1, at=xtick, labels=c("Downtime 1", "Downtime 2", "Downtime 3"), cex.axis = 1.6)
```
