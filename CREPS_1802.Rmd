---
title: "Projet CREPS"
output:
  pdf_document:
    fig_height: 4
    fig_width: 5.5
    number_sections: yes
    toc: yes
date: "`r Sys.Date()`"
subtitle: "Mise en place d'un modèle statistique pour validaton du testing TM2S"
geometry: "left=3cm,right=3cm,top=3cm,bottom=3cm"
header-includes:
- \usepackage{authblk}
- \usepackage[french]{babel}
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \renewcommand\headrulewidth{1pt}
- \fancyhead[L]{\scshape \leftmark}
- \fancyhead[R]{\bfseries Projet tutoré}
- \fancyfoot[L]{INSA Toulouse}
- \author{\bfseries Célia DULUC, Thomas NIVELET, Alexia GHOZLAND}
- \affil{4\textsuperscript{e} année MA}
- \affil{INSA Toulouse}
- \affil{\small Année universitaire 2020-2021}
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, cache=TRUE)
```

**Importation des librairies `R` nécessaires**

```{r, include=FALSE}
library(readxl)
library(ggplot2)
library(ggmosaic)
library(MASS)
library(dplyr)
library(fmsb)
library(corrplot)
```

**Importation des données** 

```{r}
# Importation du fichier de données depuis un .xlsx
donnees_creps <- read_excel("donnees_CREPS_1.xlsx")
```

# Statistiques descriptives : première prise en main des données
## Statistiques unidimensionnelles : étude générale des pôles et des scores obtenus

```{r}
# Redéfinition de la variable qualitative à bon escient (en facteurs)
donnees_creps$POLES <- as.factor(donnees_creps$POLES)

# Vérification du contenu et présentation sommaire des variables
summary(donnees_creps)
head(donnees_creps)
```

```{r}
# Observations et variables contenues dans le jeu de données
dim(donnees_creps)
```
Le premier jeu de données à notre disposition contient les scores obtenus au testing TM2S, relatif à la mobilité et à la stabilité su sportif, de 90 athlètes du CREPS, répartis dans 6 pôles sportifs différents (Aviron, Boxe française, Basketball, Bowling, Natation et Natation synchronisée). Plusieurs des variables présentes dans ce premier jeu de données attireront particulièrement notre attention dans la suite du projet :

- `SCORE`, une variable quantitative discrète, prenant des valeurs de 0 à 20, représentant le score global obtenu par le sportif au test.
- `POLES`, une variable qualitative à 6 modalités représentant les pôles du CREPS sur lesquels a été expérimenté le TM2S.
- `MOBINF`, une variable quantitative discrète, représentant le score obtenu en mobilité inférieure.
- `MOBSUP`, une variable quantitative discrète, représentant le score obtenu en mobilité supérieure.
- `STABINF`, une variable quantitative discrète, représentant le score obtenu en stabilité inférieure.
- `STABSUP`, une variable quantitative discrète, représentant le score obtenu en stabilité supérieure.

```{r}
pie(table(donnees_creps$POLES), col = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen", "darkorange2", "brown4"),main = "Répartition des athlètes dans les pôles")

barplot(table(factor(donnees_creps$SCORE, levels = 0:20)), main = "Scores obtenus - Tous pôles confondus", ylim = c(0,20), cex.names = 0.6, ylab = "Effectifs", xlab= "Score", col = heat.colors(20))
```

- Répartition des athlètes dans les pôles : 
Le diagramme circulaire ci-dessus nous permet d'affirmer que les sportifs testés proviennent de six pôles différents et mis à part en Basketball, les pôles présentent des effectifs assez comparables.
On note que c'est en Basket Ball qu'il y a le plus d'athlètes, soit un tiers de l'effectif total des sportifs du CREPS testés. Viennent ensuite, de façon décroissante, la Natation Synchronisée, la Boxe Française, l'Aviron et la Natation. C'est en bowling qu'il y a le moins d'athlètes.

- Scores globaux obtenus :
On constate que les notes sont très échelonnées de 0 à 19 et prennent quasiment la totalité des valeurs possibles. Le score le plus obtenu au test est la note de 10, soit la moyenne.
Cependant, on dénote davantage de notes inférieures à la moyenne que de notes supérieures à 10/20.
12 athlètes ont reçu un score de 8, autant que pour un score de 9. Des pics plus discrets s'affichent pour les scores de 12, 5 (et 14). 
Très peu d'athlètes obtiennent des notes au moins égales à 13.
Notre premières impression est telle que l'on imagine ce testing assez compliqué dans l'absolu ; mais il va nous falloir étudier plus en détails cette notation, par pôles et par catégories d'exercices.

## Statistiques bidimensionnelles : distinction des scores obtenus selon les pôles
### Distinction des scores globaux obtenus selon les pôles

Dans cette partie, nous cherchons à analyser les scores ou intervalles de score les plus obtenus et offrir une comparaison selon les pôles. On essaiera de déterminer une certaine variabilité des profils et de mettre en perspective ces résultats selon les pôles.

```{r}
SCORE <- donnees_creps$SCORE
POLES <- donnees_creps$POLES

tab_mean <- tapply(SCORE, POLES, mean) # Moyenne des scores pour chaque pôle
tab_median <- tapply(SCORE, POLES, median) # Médiane des scores pour chaque pôle
tab_mean
tab_median
```

```{r}
boxplot(donnees_creps$SCORE ~ donnees_creps$POLES, cex.axis = 0.8, main = "Boxplots parallèles des scores en fonction des pôles", xlab = "Pôles", ylab = "Score")
```
Les boxplots (ou diagrammes en boîtes) ci-dessus nous permettent d'évaluer la position, l'éventuelle symétrie, ainsi que la dispersion des scores obtenus pour chaque pôle. Sur ces graphiques, 50% des observations sont rassemblées à l'intérieur d'une boîte grisée, le trait plein noir représentant la médiane.

Les médianes sont alignés au score de 9/20 pour les boxplots concernant les athlètes de Bowling, Basketball et Aviron : il y a autant de scores obtenus en-dessous qu'au dessus de cette note, pour ces pôles-ci.
En aviron, on observe un faible écart inter-quartiles, les scores ne sont pas trop dispersés au sein de ce pôle, niveau homogène. (mais 2 outliers).
En Natation Synchronisée, la médiane s'élève à 11/20, la plus élevée parmi tous les pôles. De plus, l'écart interquartile n'est pas très haut, on peut en déduire un niveau homogène et bon, en majorité situé au-dessus de la moyenne de 10/20.
En conclusion, les scores médians sont plutôt proches mais les scores selon les pôles sont plus dispersés (BF par rapport à AV par exemple).
L'ensemble de ces analyses doit être mise en relief avec l'effectif d'athlètes pour chaque pôle considéré ; en effet, on dispose de moins d'athlètes pour l'aviron que pour le basketball par exemple.

```{r}
ggplot(data = donnees_creps) + geom_histogram(aes(x = SCORE, fill = POLES), binwidth = 0.5) + scale_x_continuous(breaks=seq(0, 20, by = 1)) + xlab("Score") + ylab("Effectifs")  + ggtitle("Effectifs selon les scores obtenus (toutes catégories) selon les pôles")
```
Parmi les bowlers, les moins nombreux, aucun n'obtient un score au dessus de 12, tout comme les boxeurs.
Les seuls 0 visibles correspondent aux joueurs de Basketball, mais ils sont les plus nombreux (donc davantage de possibilités de variabilité des scores).
En Natation synchronisée, les scores se situent entre 7 (élevé pour un score minimum) et 18/20.
Du fait qu'ils soient nombreux, on voit bien que les joueurs de basket ont des scores qui s'étendent de 0 à 19/20 : les niveaux peuvent être considérés comme hétérogènes, bien que l'on puisse se poser la question sur les valeurs extrêmement faibles (problème lors du test ? erreur de casting ?).

### Étude de la spécificité de chaque pôle

```{r}
par(mfrow=c(2,2))
plot(donnees_creps$MOBINF ~donnees_creps$POLES, ylab="Score en mobilite inf", xlab="Poles")
plot(donnees_creps$MOBSUP ~donnees_creps$POLES, ylab="Score en mobilite sup", xlab="Poles")
plot(donnees_creps$STABINF ~donnees_creps$POLES, ylab="Score en stabilité inf", xlab="Poles")
plot(donnees_creps$STABSUP ~donnees_creps$POLES,ylab="Score en stabilité sup", xlab="Poles")
```

- Pour la mobilité inférieure (note sur 8) : le score médian de tous les pôles se situe aux alentours de 3, i.e 50% des athlètes de ces pôles ont un score inférieur à 3. Le pôle Natation Synchronisée se détache avec une médiane très proche de 6, et une certaine homogénéité des athlètes (faible écart interquartile). (Les nageuses sont plus souples : sont-elles moins blessées à ce niveau-la pour autant?).
- Pour la mobilité supérieure (note sur 4) : Les pôles Boxe et Bowling sont les plus faibles avec une médiane de 1. On peut également remarquer qu'un grand nombre d'atlètes ont un score approximatif de 1, car la médiane semble assimilable au premier quartile. Ces deux pôles semblent avoir un comportement similaire, même dispersion (cf ont-ils le même type de blessure?).Pour l'Aviron et le Basket, il est plus difficile de conclure, la dispersion est importante, les athlètes non homogènes? Plus grande homogénéité pour la Natation et la Natation Synchronisée, avec une médiane plus haute.
- Pour la stabilité inférieure (note sur 6) : Encore une fois, on constate une certaine disparité entre les pôles. Les médianes les plus faibles (autour de 2) pour la boxe, la natation et la Natation Synchronisée. Il y a une dispersion plus important pour les trois autres pôles mais ils obtiennent globalement de meilleurs scores.
- Pour la stabilité supérieure (note sur 2) : 75% des athlètes ont une note inférieure ou égale à 1 pour le basket, la boxe et le bowling. 75% des athlètes ont une note supérieure ou égale à 1 pour l'aviron et la natation. La répartition est symétrique par rapport au score 1 pour la Natation Synchronisée.

Mais toutes ces différences constatées entre pôles sont-elles significatives ou simplement minimes? Cf les tests de significativité des paramètres avec une ANOVA pour chaque type de score.

N.B.: Les observations à l'extérieur des moustaches doivent être étudiées de plus près. Elles ne sont pas forcément des valeurs aberrantes, mais possiblement des valeurs "déterminantes". A voir...

Représentons les scores moyens obtenus pour chaque pôle dans les quatre catégories d'exercices sous la forme d'un radarplot :

```{r}
# Récupération des moyennes et définition des valeurs par catégorie
## AV
Mob_inf_AV = c(8,0,mean(donnees_creps$MOBINF[POLES=="AV"]))
Mob_sup_AV = c(4,0,mean(donnees_creps$MOBSUP[POLES=="AV"]))
Stab_inf_AV = c(6,0,mean(donnees_creps$STABINF[POLES=="AV"]))
Stab_sup_AV = c(2,0,mean(donnees_creps$STABSUP[POLES=="AV"]))
data_spider_AV <- cbind(Mob_inf_AV,Mob_sup_AV,Stab_inf_AV,Stab_sup_AV)
rownames(data_spider_AV) <- c("MAX", "MIN", "AV")
colnames(data_spider_AV) <- c("Mob. inf.", "Mob. sup.", 
                              "Stab. inf.", "Stab. sup.")
data_spider_AV = as.data.frame(data_spider_AV)

## BF
Mob_inf_BF = c(8,0,mean(donnees_creps$MOBINF[POLES=="BF"]))
Mob_sup_BF = c(4,0,mean(donnees_creps$MOBSUP[POLES=="BF"]))
Stab_inf_BF = c(6,0,mean(donnees_creps$STABINF[POLES=="BF"]))
Stab_sup_BF = c(2,0,mean(donnees_creps$STABSUP[POLES=="BF"]))
data_spider_BF <- cbind(Mob_inf_BF,Mob_sup_BF,Stab_inf_BF,Stab_sup_BF)
rownames(data_spider_BF) <- c("MAX", "MIN", "BF")
colnames(data_spider_BF) <- c("Mob. inf.", "Mob. sup.", 
                              "Stab. inf.", "Stab. sup.")
data_spider_BF = as.data.frame(data_spider_BF)

## BK
Mob_inf_BK = c(8,0,mean(donnees_creps$MOBINF[POLES=="BK"]))
Mob_sup_BK = c(4,0,mean(donnees_creps$MOBSUP[POLES=="BK"]))
Stab_inf_BK = c(6,0,mean(donnees_creps$STABINF[POLES=="BK"]))
Stab_sup_BK = c(2,0,mean(donnees_creps$STABSUP[POLES=="BK"]))
data_spider_BK <- cbind(Mob_inf_BK,Mob_sup_BK,Stab_inf_BK,Stab_sup_BK)
rownames(data_spider_BK) <- c("MAX", "MIN", "BK")
colnames(data_spider_BK) <- c("Mob. inf.", "Mob. sup.", 
                              "Stab. inf.", "Stab.sup.")
data_spider_BK = as.data.frame(data_spider_BK)

## BL
Mob_inf_BL = c(8,0,mean(donnees_creps$MOBINF[POLES=="BL"]))
Mob_sup_BL = c(4,0,mean(donnees_creps$MOBSUP[POLES=="BL"]))
Stab_inf_BL = c(6,0,mean(donnees_creps$STABINF[POLES=="BL"]))
Stab_sup_BL = c(2,0,mean(donnees_creps$STABSUP[POLES=="BL"]))
data_spider_BL <- cbind(Mob_inf_BL,Mob_sup_BL,Stab_inf_BL,Stab_sup_BL)
rownames(data_spider_BL) <- c("MAX", "MIN", "BL")
colnames(data_spider_BL) <- c("Mob. inf.", "Mob. sup.", 
                              "Stab. inf.", "Stab.sup")
data_spider_BL = as.data.frame(data_spider_BL)

## NAT
Mob_inf_NAT = c(8,0,mean(donnees_creps$MOBINF[POLES=="NAT"]))
Mob_sup_NAT = c(4,0,mean(donnees_creps$MOBSUP[POLES=="NAT"]))
Stab_inf_NAT = c(6,0,mean(donnees_creps$STABINF[POLES=="NAT"]))
Stab_sup_NAT = c(2,0,mean(donnees_creps$STABSUP[POLES=="NAT"]))
data_spider_NAT <- cbind(Mob_inf_NAT,Mob_sup_NAT,Stab_inf_NAT,Stab_sup_NAT)
rownames(data_spider_NAT) <- c("MAX", "MIN", "NAT")
colnames(data_spider_NAT) <- c("Mob. inf.", "Mob. sup.", 
                              "Stab. inf.", "Stab. sup.")
data_spider_NAT = as.data.frame(data_spider_NAT)

## NS
Mob_inf_NS = c(8,0,mean(donnees_creps$MOBINF[POLES=="NS"]))
Mob_sup_NS = c(4,0,mean(donnees_creps$MOBSUP[POLES=="NS"]))
Stab_inf_NS = c(6,0,mean(donnees_creps$STABINF[POLES=="NS"]))
Stab_sup_NS = c(2,0,mean(donnees_creps$STABSUP[POLES=="NS"]))
data_spider_NS <- cbind(Mob_inf_NS,Mob_sup_NS,Stab_inf_NS,Stab_sup_NS)
rownames(data_spider_NS) <- c("MAX", "MIN", "NS")
colnames(data_spider_NS) <- c("Mob. inf.", "Mob. sup.", 
                              "Stab. inf.", "Stab. sup.")
data_spider_NS = as.data.frame(data_spider_NS)
```


```{r}
# Préparation et affichage des graphiques

par(mfrow=c(1,2), mar = c(1, 0, 1, 0))

options(repr.plot.width = 15, repr.plot.height = 8)

radarchart(data_spider_AV, 
           axistype = 1,
           pcol = "lightskyblue2", pfcol = scales::alpha("lightskyblue2", 0.5), plwd = 2, plty = 1,
           cglcol = "grey", cglty = 1, cglwd = 0.8,
           axislabcol = "grey",
           vlcex = 0.6, vlabels = colnames(data_spider_AV),
           caxislabels = NULL, title = "Scores moyens - Aviron"
           )

radarchart(data_spider_BF, 
           axistype = 1,
           pcol = "lightgoldenrod3", pfcol = scales::alpha("lightgoldenrod3", 0.5), plwd = 2, plty = 1,
           cglcol = "grey", cglty = 1, cglwd = 0.8,
           axislabcol = "grey",
           vlcex = 0.6, vlabels = colnames(data_spider_BF),
           caxislabels = NULL, title = "Scores moyens - Boxe française"
           )

```
```{r}
par(mfrow=c(1,2), mar = c(1, 2, 2, 1))
options(repr.plot.width = 20, repr.plot.height = 10)

radarchart(data_spider_BK, 
           axistype = 1,
           pcol = "mistyrose4", pfcol = scales::alpha("mistyrose4", 0.5), plwd = 2, plty = 1,
           cglcol = "grey", cglty = 1, cglwd = 0.8,
           axislabcol = "grey",
           vlcex = 0.7, vlabels = colnames(data_spider_BK),
           caxislabels = NULL, title = "Scores moyens - Basketball"
           )

radarchart(data_spider_BL, 
           axistype = 1,
           pcol = "lightcoral", pfcol = scales::alpha("lightcoral", 0.5), plwd = 2, plty = 1,
           cglcol = "grey", cglty = 1, cglwd = 0.8,
           axislabcol = "grey",
           vlcex = 0.7, vlabels = colnames(data_spider_BL),
           caxislabels = NULL, title = "Scores moyens - Bowling"
           )
```

```{r}
par(mfrow=c(1,2), mar = c(1, 2, 2, 1))
options(repr.plot.width = 20, repr.plot.height = 10)

radarchart(data_spider_NAT, 
           axistype = 1,
           pcol = "mediumblue", pfcol = scales::alpha("mediumblue", 0.5), plwd = 2, plty = 1,
           cglcol = "grey", cglty = 1, cglwd = 0.8,
           axislabcol = "grey",
           vlcex = 0.7, vlabels = colnames(data_spider_NAT),
           caxislabels = NULL, title = "Scores moyens - Natation"
           )

radarchart(data_spider_NS, 
           axistype = 1,
           pcol = "lightseagreen", pfcol = scales::alpha("lightseagreen", 0.5), plwd = 2, plty = 1,
           cglcol = "grey", cglty = 1, cglwd = 0.8,
           axislabcol = "grey",
           vlcex = 0.7, vlabels = colnames(data_spider_NS),
           caxislabels = NULL, title = "Scores moyens \n Natation synchronisée"
           )
```

```{r,fig.height = 7, fig.width = 10}

# Test graphes araignée superposés
## Récupération des données moyennes, minimales et maximales
 

Mob_inf = c(8,0,mean(donnees_creps$MOBINF[POLES=="AV"]),mean(donnees_creps$MOBINF[POLES=="BF"]),mean(donnees_creps$MOBINF[POLES=="BK"]),mean(donnees_creps$MOBINF[POLES=="BL"]),mean(donnees_creps$MOBINF[POLES=="NAT"]),mean(donnees_creps$MOBINF[POLES=="NS"]))

 

Mob_sup = c(4,0,mean(donnees_creps$MOBSUP[POLES=="AV"]),mean(donnees_creps$MOBSUP[POLES=="BF"]),mean(donnees_creps$MOBSUP[POLES=="BK"]),mean(donnees_creps$MOBSUP[POLES=="BL"]),mean(donnees_creps$MOBSUP[POLES=="NAT"]),mean(donnees_creps$MOBSUP[POLES=="NS"]))

 

Stab_inf = c(6,0,mean(donnees_creps$STABINF[POLES=="AV"]),mean(donnees_creps$STABINF[POLES=="BF"]),mean(donnees_creps$STABINF[POLES=="BK"]),mean(donnees_creps$STABINF[POLES=="BL"]),mean(donnees_creps$STABINF[POLES=="NAT"]),mean(donnees_creps$STABINF[POLES=="NS"]))

 

Stab_sup = c(2,0,mean(donnees_creps$STABSUP[POLES=="AV"]),mean(donnees_creps$STABSUP[POLES=="BF"]),mean(donnees_creps$STABSUP[POLES=="BK"]),mean(donnees_creps$STABSUP[POLES=="BL"]),mean(donnees_creps$STABSUP[POLES=="NAT"]),mean(donnees_creps$STABSUP[POLES=="NS"]))

 

data_spider <- cbind(Mob_inf,Mob_sup,Stab_inf,Stab_sup)

rownames(data_spider) <- c("MAX", "MIN", "AV", "BF", "BK", "BL", "NAT", "NS")

colnames(data_spider) <- c("Mob. inf.", "Mob. sup.",

                              "Stab. inf.", "Stab. sup.")

data_spider = as.data.frame(data_spider)

 

## Définition et affichage du radarchart
par(mar = c(1, 1, 1, 1))
radarchart(data_spider,

           axistype = 1,

           pcol = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen"),

           plwd = 2, plty = 1,

           cglcol = "grey", cglty = 1, cglwd = 0.4,

           axislabcol = "grey",
           vlcex = 0.7, vlabels = colnames(data_spider_NAT),
           caxislabels = NULL, title = "Scores moyens par catégories d'exercices"
           )

legend(

  x = "right", legend = rownames(data_spider[-c(1,2),]), horiz = FALSE,

  bty = "n", pch = 20 , col = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen"),

  text.col = "black", cex = 1, pt.cex = 1.5

  )
 

```
On peut clairement distinguer que les athlètes de Natation Synchronisée se démarquent sur la mobilité inférieure, ce qui peut s'expliquer par la souplesse requise et améliorée dans ce sport. Tandis que, les athlètes de la Natation, eux, se démarquent en stabilité supérieure ( les membres supérieurs sont plus sollicités pour nager ). 
En Stabilité Inférieure, ce sont les athlètes de l'Aviron qui tirent leur épingle du jeu (ils obtiennent près de 75% des points). Moins élevés que les nageurs artistiques mais ils obtiennt + de 50% des points en Stabilité Supérieure ce qui paraît logique au vu des mouvements nécessaires à ce sport.
En boxe française, les scores par catégories d'exercices sont plutôt homogènes, les athlètes obtiennent autour de 50% des points dans chacune des catégories.
Au Basket Ball, les athlètes sont assez bons sur les exercices de mobilité supérieure  et stabilité inférieure puisqu'ils obtiennent près de 50% des points dans chacune des ces 2 catégories tandis que sur les autres, ils sont plutôt autour de 25%.
Les joueurs de Bowling eux, sont assez bons sur la stabilité inférieure(50% des points) alors qu'ils sont à 40% du total sur les autres catégories. 

# Construction d'un modèle linéaire : tests de modèle d'ANOVA

```{r}
ao.score.pole = lm(SCORE ~ POLES, data = donnees_creps)
summary(ao.score.pole)
```

On peut voir que le modèle d'ANOVA testé ici (la variable POLES pour expliquer le score) n'ajuste pas bien les données, car le $R^2$ est faible (proche de 0). Donc l'appartenance à un pôle ne suffit pas à expliquer le score obtenu au test. Pour mieux expliquer le score, il nous faudra donc "complexifier" le modèle", en considérant par exemple d'autres variables.

```{r}
#Modèle d'anova pour chaque catégorie de sport

ao.score_mob_inf.pole = lm(donnees_creps$MOBINF ~ POLES, data = donnees_creps)
summary(ao.score_mob_inf.pole)

ao.score_mob_sup.pole = lm(donnees_creps$MOBSUP ~ POLES, data = donnees_creps)
summary(ao.score_mob_sup.pole)

ao.score_stab_inf.pole = lm(donnees_creps$STABINF ~ POLES, data = donnees_creps)
summary(ao.score_stab_inf.pole)

ao.score_stab_sup.pole = lm(donnees_creps$STABSUP ~ POLES, data = donnees_creps)
summary(ao.score_stab_sup.pole)
```

On voit que la variable POLES explique mieux les scores en mobilité inférieure et en stabilité supérieure qu'elle n'explique le score total (meilleur $R^2$). Donc on peut en conclure que c'est dans ces deux "sous-catégories" de score que les disparités entre les pôles vont être les plus marquées. 

A Chercher : p-valeurs élevées (on ne rejette pas individuellement la nullité du paramètre) à interpréter (variables pôles n'expliquent pas/mal les scores...?)

Piste à creuser : Essayer de regarder l'importance relative des différentes sous-catégories de scores. Certaines pèsent-elles plus dans le risque de blessure?

Piste à creuser : il peut-être intéressant de faire le comparatif avec le "type" de blessure qu'il y a dans chaque pôle. Les blessures sont-elles globalement les mêmes dans les différents pôles? Ou la fréquence de certaines blessures est-elle plus importante dans certains pôles? 
Et tracer aussi comme il est fait dans le cas global, un graphe représentant les effectifs des scores spéciaux selon les pôles.

```{r}
#Possible d'essayer de mettre en place des modèles additifs voire ajouter ds interactions pour voir si le $R^2$ est meilleur ? 

#reg.mob_inf_stab_sup <- lm(donnees_creps$SCORE ~ MOBINF + STABSUP + MOBSUP, data = donnees_creps)
#summary(reg.mob_inf_stab_sup)

#reg.complet <- lm(donnees_creps$SCORE ~ MOBINF + MOBSUP + STABINF + STABSUP, data = donnees_creps)
#summary(reg.complet)
#stepAIC(reg.complet)
#anova(reg.mob_inf_stab_sup,reg.complet)
# pistes à creuser...
```


```{r}
ggplot(data = donnees_creps) + geom_histogram(aes(x = MOBINF, fill = POLES), binwidth = 0.5) + scale_x_continuous(breaks=seq(0, 20, by = 1)) + xlab("Score") + ylab("Effectifs") + ggtitle("Effectifs selon les scores obtenus en mobilité inférieure selon les pôles")
```
On retrouve ici que les résultats obtenus aux exercices de mobilité inférieure sont très disparates, les 9 niveaux de notation sont utilisés. L'essentiel des notes sont comprises entre 0 et 6, et ce, pour l'ensemble des pôles.
Alors que chaque pôle apparaît sur quasiment chaque score possible de mobilité inférieure, les athlètes de natation synchronisée n'apparaissent qu'à partir de 3/8 ! Les scores obtenus pour ces athlètes sont bien meilleurs relativement aux autres pratiquants, la mobilité inférieure étant généralement une caractéristique intrinsèque de ce sport, ces résultats pour la natation synchronisée semblent conformes à nos attentes. 

```{r}
ggplot(data = donnees_creps) + geom_histogram(aes(x = STABSUP, fill = POLES), binwidth = 0.5) + scale_x_continuous(breaks=seq(0, 20, by = 1)) + xlab("Score") + ylab("Effectifs") + ggtitle("Effectifs selon les scores obtenus en stabilité supérieure selon les pôles")
```
De même, pour la stabilité supérieure, on constate que les trois valeurs de notation sont utilisées, et que les résultats sont à nouveau uniformément disparates. Cependant, aucune tendance claire ne se dégage ici, aucun pôle n'obtient des résultats significativement plus élevés ou plus faibles ; cela étant probablement dû à la faible échelle de notation pour cette partie du test. La stabilité supérieure ne semble donc pas être une caractéristique qui dégage clairement un pôle par rapport à un autre dans le cadre de notre étude, avec les données que nous avons à disposition.

Les deux graphes ci-dessus permettent une meilleure visualisation que les boxplots présentés plus haut dans le document. 

(Partie blessure)
# Redéfinition de la variable `Score` en sous-catégories

```{r}
# Possible de redéfinir les seuils et les noms !
# Possible aussi de redéfinir le nom des facteurs !

SCORE_cat <- donnees_creps$SCORE
SCORE <- donnees_creps$SCORE
POLES <- donnees_creps$POLES

SCORE_cat[SCORE<5]<-"Tres faible"
SCORE_cat[SCORE<10 & SCORE>=5]<-"Faible"
SCORE_cat[SCORE<15 & SCORE>=10] <-"Moyen"
SCORE_cat[SCORE<=20 & SCORE>=15]<-"Bon"

SCORE_cat = factor(SCORE_cat, levels = c("Tres faible", "Faible", "Moyen", "Bon"))
```

```{r}
# Table de contingence en fonction des facteurs définis

table.cat <- table(POLES, SCORE_cat)
addmargins(table.cat)

prop.cat <- prop.table(table.cat,1)
prop.cat
```

Grâce à la table de contingence ci-dessus, on peut observer la répartition des résultats au test des différents athlètes, selon les différentes "tranches" de score qu'on a défini : Très faible, Faible, Moyen, et Bon. 

On constate qu'il n'y a que très peu de score au-dessus de 15. On peut donc d'ores et déjà se poser la question de la pertinence de ce découpage en sous-catégories de score et de ces valeurs de seuil : sont-elles vraiment adaptées compte tenu des résultats obtenus et de la construction du testing ? Ce découpage en sous catégories est une première étape dans la définition de l'obtention d'un "bon score" et d'un "moins bon score" à ce testing.

Dans la seconde table de contingence, on peut observer les répartition des scores de chaque pôle en pourcentage. Pour tous les pôles sauf le pôle de NS la plupart des athlètes ont un score compris entre 5 et 10. Pour la NS, un score moyen (i.e. entre 10 et 15) est majoritaire.

Cf illustration avec le mosaic plot ci-dessous.

Piste à creuser : éventuellement redéfinir nos différents seuils pour le score: cf en fonction de FMS, GMC, analyse des blessures... En effet, il y a beaucoup d'athlètes dans chaque pôle qui ont un score que nous avons défini comme "moyen", entre 10 et 15, et pourtant le nombre de blessures est conséquent. Mais correspondent-elles à des blessures que l'on pourrait qualifier de "mineures" ? Faire le parallèle avec les seuils considérés pour le testing FMS par exemple.

```{r}
mosaicplot(table.cat,color=c("#FF3333","#FF9933", "#FFCC00","#66CC00"), main="Contingence Catégorie de score / Pôles", xlab = "Pôles", ylab = "Catégories de score") 

```
Interprétation :
On retrouve certains points soulevés précédemment :
- Aucun score considéré comme "très faible" en natation et natation synchronisée.
- A contrario, aucun score "bon" en BF, BL, NAT. Les scores sont plutôt moyens pour ces pôles, et assez concentrés.
- Seule de meilleurs résultats globaux (moyens voire bons) en natation synchronisée, la tendance est globalement meilleure pour ce pôle relativement aux autres. 
- En natation, résultats concentrés et seulement répartis dans les facteurs "Faible" et "Moyen".
- Proportion semblable de scores très faibles en AV, BF et BK.

L'ensemble de ces interprétations nous a permis de confirmer les observations et tendances dégagées dans l'analyse descriptive uni- et bidimensionnelle. Les futurs objectifs de ce découpage en sous-catégories de score seront d'essayer de redéfinir les seuils, et de les mettre en perspective avec les seuils déjà existants (comme pour le test FMS). Nous essaierons alors de définir ce que l'on peut considérer comme un "bon score" au test, en corrélant notamment avec les données sur les blessures.

# Étude des données blessures 

*Importation et définition des données*
```{r}
donnees_bless= read_excel("Donnees_bless_Poles.xlsx")
```

```{r}
donnees_bless$POLE=as.factor(donnees_bless$POLE)
```

## Comparaison des scores obtenus et des blessures dans chaque pôle
### Première vision globale

```{r}
par(mar=c(4,4,3,5))
plot(donnees_creps$SCORE ~donnees_creps$POLES, main="Comparaison entre les scores obtenus et le pourcentage de blessés dans chaque pôle", cex.main=0.75, ylab="", xlab="", axes = FALSE,col="blue")
axis(4, ylim=c(0,20),col="blue",col.axis="blue",at=seq(0, 20, by=2), cex.axis = 0.8) 
mtext("Score global",side=4,line=2.5, col="blue") 
par(new=T)
plot(donnees_bless$BLESS_POURC ~donnees_bless$POLE, ylab="", xlab="", axes= FALSE, medcol = "red", boxcol = "red")
axis(2 , xlim=c(0,101), col="red",col.axis="red",at=seq(0, 101, by=10), cex.axis = 0.7) 
mtext("Pourcentage de blessures",side=2,line=2.5, col = "red")
axis(1,at=c(1:8), labels=c("AV", "BF", "BK", "BL", "BS" ,"NAT", "NS", "R"), cex.axis = 0.8)
mtext("Pôles",side=1,line=2.5,col="black") 
box()

# Manque les pourcentages de blessures pour les deux nouveaux pôles ! Savoir à quoi correspond l'état du sportif 
# dans le nouvel excel
```

Commentaires : 
Globalement, lorsque les différents scores obtenus par les athlètes du pôles semblent être plus faibles, le pourcentage de blessés semblent être beaucoup plus important que pour des pôles avec des scores plus élevés. (Pour le voir, comparer la tendance des pourcentages en rouges par rapport à la tendance des médianes des boxplots et en fonction de la dispersion des boxplots).

Tous les pôles, excepté celui de la NS ont des scores médians de 10, et un pourcentage de blessés supérieur à 50%. Au contraire, le pôle NS a un score médian supérieur à 10 et un pourcentage de blessé nul ! On pourrait ainsi voir un premier seuil déterminant apparaître au niveau du score 10.

Piste à creuser : Pourrait-on définir un second seuil à 8. Avoir un score en dessous de 8 (comme pour le pôle Boxe Française ici) augmenterait encore d'un cran le risque de blessures?

### En fonction du score médian (ou score moyen) de chaque pôle

Le graphe ci-après représente le nombre de blessures en fonction du score médian de chaque pôle, sous la forme d'un petit nuage de points. On y superpose une droite de régression linéaire.
```{r}
#a<- boxplot(donnees_creps$SCORE~donnees_creps$POLES)

#moda=gl(n=6, k=1, length=6, labels=c("AV", "BF", "BK", "BL", "NAT","NS" ))
#plot(a$stats[3,]~moda, main="Valeur médiane du nombre de blessés en fonction du pôle", ylab="médiane du nombre de blessures", xlab="Pôles")

#coord_x=a$stats[3,]
#coord_x=tab_median[c(1:4,6:7)]
coord_x=tab_median
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS","NAT","NS","R")
reg.med <- lm(coord_y ~ coord_x)
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score médian obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score médian obtenu pour chaque pôle", cex.main=0.7)
abline(reg.med, col = "red")
text(coord_x,coord_y,nom, pos=2, offset=0.5, cex=0.7)

#Plus pertinent avec les scores moyens de chaque pôle??
coord_x=tab_mean
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS","NAT","NS","R")
reg.moy <- lm(coord_y ~ coord_x)
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score moyen obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score moyen obtenu pour chaque pôle", cex.main=0.7)
abline(reg.moy, col = "red")
text(coord_x,coord_y,nom, pos=2, offset=0.5, cex=0.7)

# Ajustement des modèles :
summary(reg.med)
summary(reg.moy)

```
Pour le score médian: 

Il semblerait que l'on puisse grossièrement tracer une droite de régression linéaire pour ajuster les données (droite qui passe au milieu des points (1 point = 1 individu), les individus étant les pôles). On remarque que le pourcentage de blessés diminue lorsque le score augmente. (Une tendance opposée aurait été étrange !).

Cependant, pour le modèle de régression linéaire en fonction du score médian de chaque pôle, on peut remarquer que la p-valeur du test de comparaison avec le modèle blanc est assez élevée, de l'odre de 0.33. On ne peut donc pas rejeter avec le modèle blanc, ce qui ne nous permet pas de dire avec ces données qu'il existe une relation linéaire entre le score au TM2S et le pourcentage de blessés parmi les athlètes.

Pour le score moyen:

Visuellement, on pourrait également dire que la droite de régression linéaire semble ajuster les données. 

Cependant, la p-valeur du test de comparaison avec le modèle blanc est environ égale à 0.19, ce qui ne nous permet pas de rejeter le modèle blanc. PAr conséquent le modèle de régression linéaire n'ajuste pas les données.

Nb: Raisonnement à refaire avec les données Score: blessures pour chaque athlète (l'athlète est blessé 1, sinon 0 : régression logistique? au lieu du pourcantage de blessures en variables à expliquer). Regarder si un lien significatif existe! (dernière étape de la validation du protocole! le test permet-il de prédire la blessure).


On observe une large zone sans données entre 9 et 11 pour la valeur du score médian. On pourrait déjà voir qu'un score de 11 serait un assez bon score permettant un risque moindre de blessures. MAis attention les échantillons sont petits ! Est-ce que l'échantillon de la NS n'est pas "trop spécial" et donc peut-être aberrant ?

On affine notre analyse en scindant selon les catégories d'exercices.

```{r}
par(mfrow=c(2,2))

tab_median_mobinf=tapply(donnees_creps$MOBINF, donnees_creps$POLES, median)
coord_x=tab_median_mobinf
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS","NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score médian en mob inf obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score médian obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=2, offset=0.5, cex=0.7)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)

tab_median_stabsup=tapply(donnees_creps$STABSUP, donnees_creps$POLES, median)
coord_x=tab_median_stabsup
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS","NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score médian en stab sup obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score médian obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=4, offset=0.5, cex=0.7)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)

tab_median_mobsup=tapply(donnees_creps$MOBSUP, donnees_creps$POLES, median)
coord_x=tab_median_mobsup
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS","NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score médian en mob sup obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score médian obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=4, offset=0.5, cex=0.7)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)

tab_median_stabinf=tapply(donnees_creps$STABINF, donnees_creps$POLES, median)
coord_x=tab_median_stabinf
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS","NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score médian en stab inf obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score médian obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=4, offset=0.5, cex=0.7)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)
```

- Mobilité inférieure: on aurait tendance à trouver visuellement une tendance linéaire, comme pour le score global. Le pourcentage de blessés semble diminuer quand le score dans cette catégorie augmente. Cependant, une p-valeur forte pour un test de sous-modèle avec le modèle blanc nous montre que le modle linéaire n'ajuste pas les données.

- Stabilité supérieure: On obtient une p-valeur de comparaison avec le modèle blanc environ égale à 0.03. Cela ne nous permet pas de rejeter le modèle blanc avec beaucoup de confiance. Cependant, si on se fixe un seuil de risque alpha=0.05, alors on peut se permettre de rejeter le modèle blanc et donc de conclure que le modèle linéaire est plus efficace que le modèle blanc pour ajuster les données.

- Mobilité supérieure: Droite de régression linéaire décroissante. Le pourcentage de blessés semble diminuer quand le score dans cette catégorie augmente. Avec une p-valeur environ égale à 0.06, on peut tirer à peu près les mêmes conclusions que pour la stabilité supérieure: si on se fixe un seuil de risque égal à 10% (ou même à 7%), alors on peut considérer que le modèle linéaire ajuste mieux nos données que le modèle blanc.

-Stabilité inférieure: On remarque ici un résultat assez étrange. La droite de régression linéaire est croissante, ce qui impliquerait qu'un score plus élevé dans cette catégorie induirait un pourcentage de blessures plus important. Plusieurs conclusions sont alors possible:
(1) Ce score en stabilité inférieure n'est absolument pas déterminant pour des risques de blessure.
(2) Ce score est déterminant mais le jeu de données présent ne nous permet pas de conclure: résultat d'un ou de plusieurs pôles aberrants (NS par exemple avec des blessés qui ne sont pas réellement comptabilisés et un score dans les plus faibles).
(3) Ce score est déterminant mais pas forcément dans tous les sports: il peut être important d'avoir une bonne stabilité inférieure en Basket et en Aviron mais pas en Natation et Natation Synchronisée par exemple. Dans ce dernier cas, il faudrait alors considérer différents modèles de prédiction de blessures, ou alors donner différents poids aux scores des différentes catégories.
(4) Peut-être mieux de considérer la moyenne?

Pistes à creuser: Possible que certaines relations ne soient pas linéaires mais quadratiques par exemple? Cf les différentes transformations (log, cubique etc.. et les cas d'application).
+ question soulevée dans le dernier point.

### En fonction du score moyen de chaque pôle

Considérons à présent la moyenne comme caractéristique discriminante. La moyenne a l'inconvénient d'être influencée par les valeurs extrêmes (dans le cas où un petit nombre de valeurs aberrantes peut fausser la moyenne). Mais ici, on a un échantillon relativement petit ? Donc n'est-il pas mieux de prendre la moyenne ?
Les graphes ci-dessous nous montrent qu'utiliser la moyenne peut se révéler adapté dans notre cas.
La médiane sert aussi pour des distributions asymétriques (ce qu'on a ici en général d'après les boxplots).
```{r}
par(mfrow=c(2,2))

tab_mean_mobinf=tapply(donnees_creps$MOBINF, donnees_creps$POLES, mean)
coord_x=tab_mean_mobinf
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS", "NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score moyen en mob inf obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score moyen obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=2, offset=0.5, cex=0.7)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)

tab_mean_stabsup=tapply(donnees_creps$STABSUP, donnees_creps$POLES, mean)
coord_x=tab_mean_stabsup
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS", "NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score moyen en stab sup obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score moyen obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=4, offset=0.5, cex=0.7)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)

tab_mean_mobsup=tapply(donnees_creps$MOBSUP, donnees_creps$POLES, mean)
coord_x=tab_mean_mobsup
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS", "NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score moyen en mob sup obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score moyen obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=4, offset=0.5, cex=0.7)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)

tab_mean_stabinf=tapply(donnees_creps$STABINF, donnees_creps$POLES, mean)
coord_x=tab_mean_stabinf
coord_y=donnees_bless$BLESS_POURC
nom=c("AV","BF","BK","BL","BS", "NAT","NS","R")
plot(coord_x,coord_y, ylab="Pourcentage de blessés", xlab="Score moyen en stab inf obtenu dans chaque pôle", main="Pourcentage de blessés en fonction du score moyen obtenu pour chaque pôle", cex.main=0.6)
text(coord_x,coord_y,nom, pos=4, offset=0.5, cex=0.5)
reg <- lm(coord_y ~ coord_x)
abline(reg, col = "red")
summary(reg)
```

Questions: Il serait peut-être intéressant d'avoir les blessures classées par type de blessures. Par zone comme les zones du score (mobinf, mobsup, stabinf, stabsup...).
Aussi éventuellement classées par ordre de gravité (ou selon le temps d'arrêt ?) (comme expliqué ci-dessus avant le mosaic plot).


## Étude de l'apparition de blessures en fonction de la nature du sport

On définit la nouvelle variable `Contact` dans notre jeu de données, permettant de savoir si le sport pratiqué par l'individu en question est un sport de contact ou non.
Dans notre cas, les sports considérés comme sports de contact sont la boxe française et le basket-ball.
Cette nouvelle distinction entre les sports nous permettra d'identifier d'éventuelles corrélations avec les scores obtenues et les répercussions sur les blessures observées chez certains athlètes.

```{r}
donnees_creps$Contact=donnees_creps$POLES=="BK"|donnees_creps$POLES=="BF"|donnees_creps$POLES=="R"
donnees_creps
```
```{r}
AC<-(sum(donnees_creps$Contact)/count(donnees_creps))*100

SC<-(sum(donnees_creps$Contact==FALSE)/count(donnees_creps))*100

pie(table(donnees_creps$Contact),labels=c("Sans contact","Avec contacts"), main="Répartition des athlètes selon la nature du sport : avec/sans contact")

```
```{r}
ggplot(data = donnees_creps) + geom_histogram(aes(x = SCORE, fill = Contact), binwidth = 0.5) + scale_x_continuous(breaks=seq(0, 20, by = 1)) + xlab("Score") + ylab("Effectifs") + ggtitle("Effectifs selon les scores obtenus (toutes catégories) selon l'absence ou non de contact") + theme(plot.title =element_text(,,,12)) 
```
Ce sont les athlètes de sports de contact que l'on retrouve le plus dans les scores très faibles, c'est-à-dire inférieurs à 5.
Entre 6 et 13 (15??) (inclus) ce sont plutôt les athlètes de sports sans contact que l'on retrouve en majorité (mais équitables pour les scores de 8 et 10).
Plus de 13 : bien répartis. Autant d'athlètes SC qui ont 18 que d'athlètes AC cumulés qui ont 16 ou 19 ; proportions quasi équitables en 14.

IDEE :
Nouveaux seuils ?? 0-5 ; 6-13 : 13-20 ?... 
Ou proposer 2 catégories de seuils selon AC/SC : et ainsi le jugement serait différent 
(ex : "faible" pour un basketteur(AC) serait "très faible" pour une nageuse synchr(SC))
On pourrait définir de nouveaux seuils pour de nouvelles catégories de scores selon la nature du sport (avec / sans contacts).

```{r}
boxplot(donnees_creps$SCORE ~ donnees_creps$Contact, cex.axis = 0.8, main = "Boxplots parallèles des scores en fonction de l'absence ou non de contact", 
        cex.main = 0.9,
        ylim=c(0,20),
        xlab = "Contact", ylab = "Score",
        names=c("Non","Oui"))
```
D'un point de vue global ici, les scores obtenus par les sportifs "sans contact" sont plus élevés que ceux obtenus par les sportifs "avec contacts", comme en témoigne la position des deux boîtes grises.
On note que les scores des sports sans contact sont plus élevés : 50% des scores sont entre 9 et 12, la médiane est à 9.5 tandis que 50% des scores des sports de contact (BF,BK,R) s'étendent entre 5 et 10, avec une médiane à 8.
75% des athlètes en sports AC ont moins de 10/20.
75% des athlètes en sports SC ont plus de 8/20.
Cohérence avec plot du dessus.

```{r}
par(mfrow=c(2,2))
boxplot(donnees_creps$MOBINF ~donnees_creps$Contact, ylab="Score en mobilite inf", xlab="Contact",names=c("Non","Oui"),main="Boxplots parallèles des scores en mob inf selon le contact",cex.main=0.7)
boxplot(donnees_creps$MOBSUP ~donnees_creps$Contact, ylab="Score en mobilite sup", xlab="Contact",names=c("Non","Oui"),main="Boxplots parallèles des scores en mob sup selon le contact",cex.main=0.7)
boxplot(donnees_creps$STABINF ~donnees_creps$Contact, ylab="Score en stabilité inf", xlab="Contact",names=c("Non","Oui"),main="Boxplots parallèles des scores en stab inf selon le contact",cex.main=0.7)
boxplot(donnees_creps$STABSUP ~donnees_creps$Contact,ylab="Score en stabilité sup", xlab="Contact",names=c("Non","Oui"),main="Boù xplots parallèles des scores en stab sup selon le contact",cex.main=0.7)
```
En mobilité inf :médiane AC =2 // médiane SC=3
75% des athlètes en sports AC ont moins de 4/8 : seulement 25% ont au dessus de la moyenne.
Tandis que : 50% des athlètes de sports SC ont au moins 3/8.
En mobilité sup : médiane AC =1 // médiane SC=2 ; 
En stabilité inf : médianes alignées à 3. 50% des scores sont entre 1 et 4/6 pour les AC et 50% des scores sont entre 2 et 5 ppur les athlètes des sports SC.
50% des athlètes de sports AC ont moins de 2/6.
(A revoir cette section stab inf)
En Stabilité sup : médianes à 1 quelque soit la catégorie (Avec Cont/SC). Médiane confondue avec 3ième quartile pour les athlètes des sports AC et avec 1er quartile pour athlètes des sports SC.

```{r}
Contact=donnees_creps$Contact
tab_mean2 <- tapply(SCORE, Contact, mean) # moyenne des scores pour chaque pôle
tab_median2 <- tapply(SCORE, Contact, median) # médiane des scores pour chaque pôle
tab_mean2
tab_median2
```
```{r}
donnees_creps$Contact=as.factor(donnees_creps$Contact)

ao.score.contact = lm(SCORE ~ Contact, data = donnees_creps)
summary(ao.score.contact)
```
Test de significativité de la varible Contact: p-valeur=0.00634. On rejette pas avec certitude la nullité du paramètre. Donc la variable Contact semble être utile pour expliquer le score global.
Mais R^2 très bas? donc modèle pas bon du tout????


```{r}
#essai
ao.score_mob_inf.contact = lm(donnees_creps$MOBINF ~ Contact, data = donnees_creps)
summary(ao.score_mob_inf.contact)

```
R^2 très bas...
Mais p-valeur inférieure à 5% donc on rejette la nullité des paramètres! Donc bon signe? car veut dire que la variable contact est utile pour expliquer la différence des scores?



```{r}
donnees_bless$Contact=donnees_bless$POLE=="BK"|donnees_bless$POLE=="BF"|donnees_bless$POLE=="R"
donnees_bless$Contact=as.factor(donnees_bless$Contact)
donnees_creps$Contact=as.factor(donnees_creps$Contact)
```

```{r}
plot(donnees_creps$SCORE ~donnees_creps$Contact, main="Score global en fonction du type de pôle", labels=c("Sans contact", "Avec contact"), ylab="Score global", xlab="Type de pôle")
plot(donnees_bless$BLESS_POURC ~donnees_bless$Contact, main="Pourcentage de blessures en fonction du type de pôle", labels=c("Sans contact", "Avec contact"), ylab="Pourcentage de blessures", xlab="Type de pôle")
```

```{r}
par(mar=c(4,4,3,5))
plot(donnees_bless$BLESS_POURC ~donnees_bless$Contact, ylab="", xlab="", axes= FALSE, medcol = "red", boxcol = "red")
axis(2 , xlim=c(0,100), col="red",col.axis="red",at=seq(0, 101, by=10), cex.axis = 0.7) 
mtext("Pourcentage de blessures",side=2,line=2.5, col = "red")
axis(1,at=c(1:2), labels=c("Sans contact", "Avec contact"), cex.axis = 0.8)
mtext("Types de Pôles",side=1,line=2.5,col="black") 
par(new=T)
plot(donnees_creps$SCORE ~donnees_creps$Contact, main="Comparaison entre les scores obtenus et le pourcentage de blessés dans chaque type de pôle", cex.main=0.75, ylab="", xlab="", axes = FALSE,col="blue")
axis(4, ylim=c(0,20),col="blue",col.axis="blue",at=seq(0, 20, by=2), cex.axis = 0.8) 
mtext("Score global",side=4,line=2.5, col="blue") 
box()
```

On a pu voir avec les anova effectuées juste avant qu'il y avait un lien entre le score obtenu et le type de pôle.
Graphiquement une légère corrélation semble apparaître. Est-elle légitime/significative?

```{r}
ao.score.contact = lm(BLESS_POURC ~ Contact, data = donnees_bless)
summary(ao.score.contact)
```
Mais On voit que la p-valeur du test de significativité de la variable Contact est grande. Donc on ne peut pas rejeter l'hypothèse de sa nullité. La variable Contact ne peut pas expliquer le pourcentage de blessure. On peut aussi le voir avec la p-valeur du F-test de comparaison avec le modèle blanc qui est elle aussi très grande!

Donc: cf si on obtient les mêmes conclusions avec les nouvelles données et le temps d'indispo par pôles au lieu du pourcentage de blessure! (fichier avec tous les détails par personne et faire un total et ramener en pourcentage par rapport au nombre de personnes dans la pôle). Si la variable contact n'a toujours aucune influence, alors ne sert vraiment pas à expliquer la blessure.

PB ici pour cette anova: on a que deux individus "avec contact", et quatre individus "sans contact", ce qui ne fait vraiment pas bcp..

Mais est-ce que ce serait tout de même justifié de changer sensiblement les seuils de catégorisation des scores en fonction de cette variable? (comme Alexia avait suggéré). Piste à creuser, remarquer si il y a une différence dans les scores (cela semble être le cas avec les anova), et une différence dans le "score qui suffit à se blesser"?


Faire une ANOVA à plusieurs facteurs?! Avec le facteur Pole et le facteur Contact?! Pour expliquer le score? et les blessures?
Pour expliquer les blessures: en fait c'est pas juste de faire une anova, car le pourcentage de blessures est calculé par pôle, donc faire une anova du pourcentage de blessures par pole en fonction des poles c'est pas logique ? plutôt une régression linéaire?? (ok je m'embrouille je crois je reprendrai ça à tête reposée)
Mais si on fait ça avec les temps d'indispo sur le nouveau fichier du coup y a plus ce problème! A faire..

Transformer le score en variable explicative! Car jusqu'ici on a simplement comparer des études réalisées séparément sur les variables réponses correspondant au score et à la blessure.
Pb: sans le score par individu c'est compliqué..
Mais il serait possible de construire une variable correspondant au "score moyen par pôle"? et d'éventuellement faire une ANCOVA pour la variable réponse "Temps d'indispo" (par pôle), avec les variables explicatives (Pôle, Score moyen)? ou pas logique du tout?

Donc travail avec les nouvelles données à faire!

+cf cette histoire de seuils à fixer? de manière empirique? avec les nouvelles données?










```{r}
donnees_indispo <- read_excel("Donnees_Indispo.xlsx")
```

```{r}
suppressWarnings({
donnees_indispo$IND=donnees_indispo$TOT_INDISPO
donnees_indispo$IND[donnees_indispo$TOT_INDISPO==0]="0"
donnees_indispo$IND[donnees_indispo$TOT_INDISPO>0 & donnees_indispo$TOT_INDISPO<=1]="Inf à 1" 
donnees_indispo$IND[donnees_indispo$TOT_INDISPO>1& donnees_indispo$TOT_INDISPO<=3 ]="1 à 3"
donnees_indispo$IND[donnees_indispo$TOT_INDISPO>3]="Sup à 3"

donnees_indispo$IND = factor(donnees_indispo$IND, levels = c("Sup à 3", "1 à 3", "Inf à 1", "0"))

head(donnees_indispo)})
```


```{r}

donnees_creps$SCORE_cat2 <- donnees_creps$SCORE
SCORE <- donnees_creps$SCORE
POLES <- donnees_creps$POLES

donnees_creps$SCORE_cat2[SCORE<6]<-"Faible"
donnees_creps$SCORE_cat2[SCORE<14 & SCORE>=6] <-"Moyen"
donnees_creps$SCORE_cat2[SCORE<=20 & SCORE>=14]<-"Bon"

donnees_creps$SCORE_cat2 = factor(donnees_creps$SCORE_cat2, levels = c("Faible", "Moyen", "Bon"))

#donnees_creps$SCORE_cat3 <- donnees_creps$SCORE

#donnees_creps$SCORE_cat3[SCORE<8]<-"Faible"
#donnees_creps$SCORE_cat3[SCORE<14 & SCORE>=8] <-"Moyen"
#donnees_creps$SCORE_cat3[SCORE<=20 & SCORE>=14]<-"Bon"

#donnees_creps$SCORE_cat3 = factor(donnees_creps$SCORE_cat3, levels = c("Faible", "Moyen", "Bon"))

donnees_creps$SCORE_cat3 <- donnees_creps$SCORE

donnees_creps$SCORE_cat3[SCORE<5]<-"Faible"
donnees_creps$SCORE_cat3[SCORE<10 & SCORE>=5] <-"Moyen"
donnees_creps$SCORE_cat3[SCORE<15 & SCORE>=10]<-"Bon"
donnees_creps$SCORE_cat3[SCORE<=20 & SCORE>=15]<-"Très Bon"

donnees_creps$SCORE_cat3 = factor(donnees_creps$SCORE_cat3, levels = c("Faible", "Moyen", "Bon", "Très Bon"))

par (mfrow=c(1,3))
table.IND <- table(donnees_indispo$POLE, donnees_indispo$IND)
addmargins(table.IND)
mosaicplot(table.IND,color=c("#FF3333","#FF9933", "#FFCC00","#66CC00"), main="Contingence Niv d'indispo / Pôles", xlab = "Pôles", ylab = "Catégories d'indispo") 


table.cat2 <- table(donnees_creps$POLES, donnees_creps$SCORE_cat2)
addmargins(table.cat2)
mosaicplot(table.cat2,color=c("#FF3333","#FF9933","#66CC00"), main="Contingence Catégorie de score / Pôles", xlab = "Pôles", ylab = "Catégories de scores")

table.cat3 <- table(donnees_creps$POLES, donnees_creps$SCORE_cat3)
addmargins(table.cat3)
mosaicplot(table.cat3,color=c("#FF3333","#FF9933", "#FFCC00","#66CC00"), main="Contingence Catégorie de score / Pôles", xlab = "Pôles", ylab = "Catégories de scores")
```

```{r}
suppressWarnings({
donnees_indispo$IND_BLESS=donnees_indispo$NB_BLESS
donnees_indispo$IND_BLESS[donnees_indispo$NB_BLESS==0]="0"
donnees_indispo$IND_BLESS[donnees_indispo$NB_BLESS>0 & donnees_indispo$NB_BLESS<=1]="1" 
donnees_indispo$IND_BLESS[donnees_indispo$NB_BLESS>1& donnees_indispo$NB_BLESS<=3 ]="1 à 3"
donnees_indispo$IND_BLESS[donnees_indispo$NB_BLESS>3]="Sup à 3"

donnees_indispo$IND_BLESS = factor(donnees_indispo$IND_BLESS, levels = c("Sup à 3", "1 à 3", "1", "0"))

head(donnees_indispo)})
```

```{r}
SCORE <- donnees_creps$SCORE
POLES <- donnees_creps$POLES


donnees_creps$SCORE_cat3 <- donnees_creps$SCORE

donnees_creps$SCORE_cat3[SCORE<=5]<-"Faible"
donnees_creps$SCORE_cat3[SCORE<=10 & SCORE>5] <-"Moyen"
donnees_creps$SCORE_cat3[SCORE<15 & SCORE>10]<-"Bon"
donnees_creps$SCORE_cat3[SCORE<=20 & SCORE>=15]<-"Très Bon"

donnees_creps$SCORE_cat3 = factor(donnees_creps$SCORE_cat3, levels = c("Faible", "Moyen", "Bon", "Très Bon"))

par (mfrow=c(1,3))
table.IND <- table(donnees_indispo$POLE, donnees_indispo$IND_BLESS)
addmargins(table.IND)
mosaicplot(table.IND,color=c("#FF3333","#FF9933", "#FFCC00","#66CC00"), main="Contingence Niv d'indispo / Pôles", xlab = "Pôles", ylab = "Catégories d'indispo") 

table.cat3 <- table(donnees_creps$POLES, donnees_creps$SCORE_cat3)
addmargins(table.cat3)
mosaicplot(table.cat3,color=c("#FF3333","#FF9933", "#FFCC00","#66CC00"), main="Contingence Catégorie de score / Pôles", xlab = "Pôles", ylab = "Catégories de scores")
```

```{r}
SCORE <- donnees_creps$SCORE
POLES <- donnees_creps$POLES

suppressWarnings({
donnees_indispo$IND_BLESS=donnees_indispo$NB_BLESS
donnees_indispo$IND_BLESS[donnees_indispo$NB_BLESS>=0 & donnees_indispo$NB_BLESS<=1]="Inf a 1" 
donnees_indispo$IND_BLESS[donnees_indispo$NB_BLESS>1& donnees_indispo$NB_BLESS<3 ]="1 à 3"
donnees_indispo$IND_BLESS[donnees_indispo$NB_BLESS>=3]="Sup à 3"

donnees_indispo$IND_BLESS = factor(donnees_indispo$IND_BLESS, levels = c("Sup à 3", "1 à 3", "Inf a 1"))

head(donnees_indispo)})


donnees_creps$SCORE_cat3 <- donnees_creps$SCORE

donnees_creps$SCORE_cat3[SCORE<7]<-"Faible"
donnees_creps$SCORE_cat3[SCORE<10 & SCORE>=7] <-"Moyen"
donnees_creps$SCORE_cat3[SCORE<=20 & SCORE>=10]<-"Bon"

donnees_creps$SCORE_cat3 = factor(donnees_creps$SCORE_cat3, levels = c("Faible", "Moyen", "Bon"))

par (mfrow=c(1,3))
table.IND <- table(donnees_indispo$POLE, donnees_indispo$IND_BLESS)
addmargins(table.IND)
mosaicplot(table.IND,color=c("#FF3333", "#FFCC00","#66CC00"), main="Contingence Niv d'indispo / Pôles", xlab = "Pôles", ylab = "Catégories d'indispo") 

table.cat3 <- table(donnees_creps$POLES, donnees_creps$SCORE_cat3)
addmargins(table.cat3)
mosaicplot(table.cat3,color=c("#FF3333", "#FFCC00","#66CC00"), main="Contingence Catégorie de score / Pôles", xlab = "Pôles", ylab = "Catégories de scores")
```



```{r,fig.height = 7, fig.width = 10}

# Test graphes araignée superposés
## Récupération des données moyennes, minimales et maximales
POLE=donnees_indispo$POLE 

#NB_BLESS = c(max(donnees_indispo$NB_BLESS),min(donnees_indispo$NB_BLESS),mean(donnees_indispo$NB_BLESS[POLES=="AV"]),mean(donnees_indispo$NB_BLESS[POLE=="BF"]),mean(donnees_indispo$NB_BLESS[POLE=="BK"]),mean(donnees_indispo$NB_BLESS[POLE=="BL"]),mean(donnees_indispo$NB_BLESS[POLE=="NAT"]),mean(donnees_indispo$NB_BLESS[POLE=="NS"]))
 

Indispo_1 = c(max(donnees_indispo$NB_INDISPO_1),min(donnees_indispo$NB_INDISPO_1),mean(donnees_indispo$NB_INDISPO_1[POLE=="AV"]),mean(donnees_indispo$NB_INDISPO_1[POLE=="BF"]),mean(donnees_indispo$NB_INDISPO_1[POLE=="BK"]),mean(donnees_indispo$NB_INDISPO_1[POLE=="BL"]), mean(donnees_indispo$NB_INDISPO_1[POLE=="BS"]),mean(donnees_indispo$NB_INDISPO_1[POLE=="NAT"]),mean(donnees_indispo$NB_INDISPO_1[POLE=="NS"]), mean(donnees_indispo$NB_INDISPO_1[POLE=="R"]))
 

Indispo_2 = c(max(donnees_indispo$NB_INDISPO_2),min(donnees_indispo$NB_INDISPO_2),mean(donnees_indispo$NB_INDISPO_2[POLE=="AV"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="BF"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="BK"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="BL"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="BS"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="NAT"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="NS"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="R"]))
 

Indispo_3 = c(max(donnees_indispo$NB_INDISPO_3),min(donnees_indispo$NB_INDISPO_3),mean(donnees_indispo$NB_INDISPO_3[POLE=="AV"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="BF"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="BK"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="BL"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="BS"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="NAT"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="NS"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="R"]))


#data_spider <- cbind(NB_BLESS,Indispo_1,Indispo_2,Indispo_3)
data_spider <- cbind(Indispo_1,Indispo_2,Indispo_3)

rownames(data_spider) <- c("MAX", "MIN", "AV", "BF", "BK", "BL", "BS", "NAT", "NS", "R")

#colnames(data_spider) <- c("NB_BLESS", "Indispo_1","Indispo_2", "Indispo_3")
colnames(data_spider) <- c("Indispo_1","Indispo_2", "Indispo_3")

data_spider = as.data.frame(data_spider)

## Définition et affichage du radarchart
par(mar = c(1, 1, 1, 1))

radarchart(data_spider,axistype = 1,pcol = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen"),plwd = 2, plty = 1,cglcol = "grey", cglty = 1, cglwd = 0.4,
axislabcol = "grey",vlcex = 0.7, vlabels = colnames(data_spider),caxislabels = NULL, title = "Répartitions des indispos par pôle")

legend(x = "right", legend = rownames(data_spider[-c(1,2),]), horiz = FALSE,bty = "n", pch = 20 , col = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen"),text.col = "black", cex = 1, pt.cex = 1.5)
```
Sur ce radar chart, on peut voir la répartition des blessures selon les pôles. On remarque notamment par exemple que les boxeurs se blessent souvent (nombre de blessures supérieur à tous les autres pôles), mais principalement sur des blessures de type 1 (blessures qui engendrent moins de temps d'indisponibilité).
Pour la Natation et la Natation Synchronisée, la répartition des blessures selon leur type semble plutôt uniforme. 
En ce qui concerne le bowling, il y a plus de blessures de type 2 (blessures "moyennes").

```{r}
Moy_ind_1= c(mean(donnees_indispo$NB_INDISPO_1[POLE=="AV"]),mean(donnees_indispo$NB_INDISPO_1[POLE=="BF"]),mean(donnees_indispo$NB_INDISPO_1[POLE=="BK"]), mean(donnees_indispo$NB_INDISPO_1[POLE=="BL"]), mean(donnees_indispo$NB_INDISPO_1[POLE=="BS"]), mean(donnees_indispo$NB_INDISPO_1[POLE=="NAT"]), mean(donnees_indispo$NB_INDISPO_1[POLE=="NS"]), mean(donnees_indispo$NB_INDISPO_1[POLE=="R"]))
print(Moy_ind_1)

Moy_ind_2= c(mean(donnees_indispo$NB_INDISPO_2[POLE=="AV"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="BF"]),mean(donnees_indispo$NB_INDISPO_2[POLE=="BK"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BL"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BS"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="NAT"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="NS"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="R"]))
print(Moy_ind_2)

Moy_ind_3= c(mean(donnees_indispo$NB_INDISPO_3[POLE=="AV"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="BF"]),mean(donnees_indispo$NB_INDISPO_3[POLE=="BK"]), mean(donnees_indispo$NB_INDISPO_3[POLE=="BL"]), mean(donnees_indispo$NB_INDISPO_3[POLE=="BS"]), mean(donnees_indispo$NB_INDISPO_3[POLE=="NAT"]), mean(donnees_indispo$NB_INDISPO_3[POLE=="NS"]), mean(donnees_indispo$NB_INDISPO_3[POLE=="R"]))
print(Moy_ind_3)

abscisse=c(Moy_ind_1, Moy_ind_2, Moy_ind_3)
abscisse

ordonnee_AV=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="AV"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="AV"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="AV"]))
ordonnee_BF=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="BF"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BF"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BF"]))
ordonnee_BK=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="BK"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BK"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BK"]))
ordonnee_BL=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="BL"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BL"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BL"]))
ordonnee_BS=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="BS"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BS"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="BS"]))
ordonnee_NAT=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="NAT"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="NAT"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="NAT"]))
ordonnee_NS=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="NS"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="NS"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="NS"]))
ordonnee_R=c(mean(donnees_indispo$NB_INDISPO_1[POLE=="R"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="R"]), mean(donnees_indispo$NB_INDISPO_2[POLE=="R"]))
#plot(ordonnee_AV ~ seq(1:3), type="l", col="red")
#abline(ordonne_BF ~seq(1:3), type="l", col="orange")


x=matrix(data=c(1,2,3), nrow=3, ncol=1)
y=matrix(data=c(ordonnee_AV, ordonnee_BF, ordonnee_BK, ordonnee_BL, ordonnee_BS, ordonnee_NAT, ordonnee_NS, ordonnee_R), nrow = 3, ncol = 8)

#par(mar=c(5,5,2,2))
matplot(x,y,type="l", ylim=range(y), col=c("black","red", "green3","blue","cyan","magenta", "yellow","gray"), lty=1:3, xaxt="n", ylab="Moyenne du nombre de blessures", xlab="Type d'indisponibilité", main="Moyenne du nombre de blessures par athlète d'un pôle selon le type d'indisponibilité", cex.main=0.8)
legend('topright',legend = c("AV", "BF", "BK", "BL", "BS", "NAT", "NS", "R"), col = c("black","red", "green3","blue","cyan","magenta", "yellow","gray"), lty = 1:3, lwd = 1 )
xtick=seq(1,3, by=1)
axis(side=1, at=xtick, labels=c("Indispo 1", "Indispo 2", "Indispo 3"))
```

```{r}
mean(donnees_indispo$NB_INDISPO_2[POLE=="BK"]+donnees_indispo$NB_INDISPO_1[POLE=="BK"]+donnees_indispo$NB_INDISPO_3[POLE=="BK"])

mean(donnees_indispo$NB_INDISPO_2[POLE=="BF"]+donnees_indispo$NB_INDISPO_1[POLE=="BF"]+donnees_indispo$NB_INDISPO_3[POLE=="BF"])
```


PLAN
#Stats descpritives

#Validation du protocole
## Corrélations entre exercices et analyse de student
## Partie blessure : étude descriptive et régression linéaire
Etude descriptive: 
- quel type de blessure selon les pôles?
- quelle indisponibilité moyenne?
- quelle fréquence?
Regression lineaire:
droites de regression lineaire : pour montrer qu'il y a bien un lien entre le score obtenu et les blessures
ligne 550-600 environ cf 
## Definition des seuils

#Prediction de la blessure
Repartition en classes? Puis attribution d'un individu à une classe

# Ouverture / Limites
Sans/Avec contact : adpatation du test



