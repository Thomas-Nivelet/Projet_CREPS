...
```{r, echo=FALSE}
pie(table(donnees_creps$POLES), col = c("lightskyblue2", "lightgoldenrod3", "mistyrose4", "lightcoral", "mediumblue", "lightseagreen", "darkorange2", "brown4"),main = "Athletes distribution by poles")

barplot(table(factor(donnees_creps$SCORE, levels = 0:20)), main = "Obtained scores - all sports combined ", ylim = c(0,20), cex.names = 0.6, ylab = "Number", xlab= "Score", col = heat.colors(20))
```
...
```{r, echo=FALSE}
boxplot(SCORE ~ POLES, cex.axis = 0.8, main = "Score against poles Box Plots", xlab = "Poles", ylab = "Score", ylim = c(0,20))
```
...
```{r, echo=FALSE}
par(mfrow=c(2,2))
plot(donnees_creps$MOBINF ~donnees_creps$POLES, ylab="Lower mobility score", xlab="Poles", cex.axis = 0.85)
plot(donnees_creps$MOBSUP ~donnees_creps$POLES, ylab="Upper mobilty score", xlab="Poles", cex.axis = 0.85)
plot(donnees_creps$STABINF ~donnees_creps$POLES, ylab="Lower stability score", xlab="Poles", cex.axis = 0.85)
plot(donnees_creps$STABSUP ~donnees_creps$POLES,ylab="Upper stability score", xlab="Poles", cex.axis = 0.85)
mtext("Score versus poles Box Plots per exercises category",side = 3, line = -1,font=2, outer = TRUE)
```
...
corrplot(E,method="ellipse",number.cex=0.5, main="correlogram between each testing exercise", cex.main=0.8)
```
...
```
```{r}
ggplot(data = donnees_creps) + geom_histogram(aes(x = SCORE, fill = Contact), binwidth = 0.5) + scale_x_continuous(breaks=seq(0, 20, by = 1)) + xlab("Score") + ylab("Effectifs") + ggtitle("Athletes number versus obtained score (all poles) according to contact presence or not") + theme(plot.title =element_text(,,,12)) 
```
...
```{r}
boxplot(donnees_creps$SCORE ~ donnees_creps$Contact, cex.axis = 0.8, main = "Scores versus pole type Box Plots", 
        cex.main = 0.9,
        ylim=c(0,20),
...
mtext("Score versus pole type Box plots per exercises category",side = 3, line = -1,font=2, outer = TRUE)
```
...
```{r}
plot(donnees_creps$SCORE ~donnees_creps$Contact, main="Score versus pole type Box plots", labels=c("No", "Yes"), ylab="Score", xlab="Contact")
plot(donnees_bless$BLESS_POURC ~donnees_bless$Contact, main="Injuries percentage versus type pole Box plots", labels=c("No", "Yes"), ylab="Injuries percentage", xlab="Contact")
```
...
#par(new=T)
plot(donnees_creps$SCORE ~donnees_creps$Contact, main="Comparison between Score and injured people percentage by pole type", cex.main=0.75, ylab="", xlab="", axes = FALSE,col="blue")
